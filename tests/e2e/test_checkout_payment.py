...
class TestCheckoutUi:
    ...
    def _prepare_cart(self, driver) -> None:
        """加购并进入购物车。"""
        ProductPage(driver).open_product().add_to_cart(1)  # 加购
        ProductPage(driver).go_cart()  # 去购物车

    def test_chk_001_checkout_shows_product_and_amount(self, logged_in_driver, log_test_name):
        """CHK-001：结算页展示商品与金额。"""
        self._prepare_cart(logged_in_driver)  # 准备购物车
    ...
    def test_chk_002_submit_order_goes_payment(self, logged_in_driver, log_test_name):
        """CHK-002：提交订单跳转支付页。"""
        self._prepare_cart(logged_in_driver)  # 有车
    ...
    def test_chk_003_price_change_popup_confirm_continue(self, logged_in_driver, log_test_name):
...
class TestPaymentUi:
    ...
    def _goto_payment_via_checkout(self, driver) -> None:
        """加购 -> 结算 -> 提交到支付页。"""
        ProductPage(driver).open_product().add_to_cart(1)  # 加购
        ProductPage(driver).go_cart()  # 购物车
        CartPage(driver).go_checkout()  # 结算
        CheckoutPage(driver).submit_order()  # 提交

    @pytest.mark.flaky(reruns=2, reruns_delay=1)  # 支付页加载可能慢
    def test_pay_001_payment_page_shows_pending_or_pay(self, logged_in_driver, log_test_name):
        """PAY-001：支付页展示待支付或支付相关信息。"""
        self._goto_payment_via_checkout(logged_in_driver)  # 到支付页
    ...
    def test_pay_002_cancel_payment_can_retry(self, logged_in_driver, log_test_name):
        """PAY-002：取消支付后仍可识别待支付状态。"""
        self._goto_payment_via_checkout(logged_in_driver)  # 支付页
    ...
    def test_pay_003_payment_amount_displayed(self, logged_in_driver, log_test_name):
        """PAY-003：支付页展示金额信息。"""
        self._goto_payment_via_checkout(logged_in_driver)  # 支付页
